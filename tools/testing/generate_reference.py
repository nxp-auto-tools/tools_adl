    ...
    for dirpath, dirnames, filenames in os.walk(tests_directory):
        for filename in filenames:
            if filename.endswith(".asm"):
                ...
                instruction_name = os.path.splitext(
                    os.path.basename(source_asm_file_path)
                )[0]

                # A regular expression to match instruction lines
                instruction_pattern = r"^\t" + re.escape(
                    instruction_syntaxName_dict[instruction_name]
                )
                ...
                with open(source_asm_file_path, "r") as source_asm_file:
                    ...
                    for line in matching_lines:
                        ...
                        syntax_operands_values = re.findall(
                            r"[\w_]+", str(syntax_operands)
                        )
                        line_operands_values = re.findall(r"[\w_]+", str(line_operands))
                        operand_values_dict = {
                            key: value
                            for key, value in zip(
                                syntax_operands_values, line_operands_values
                            )
                        }
                        # Calculate reference for each instruction line
                        reference = 0
                        instruction_fields_info = instruction_fields_dict[
                            syntax_instruction
                        ]
                        for item in reversed(instruction_fields_info):
                            ...
                            for i, range in enumerate(
                                reversed(instrfield_range_dict[instrfield])
                            ):
                                ...
                                if instrfield_value in operand_values_dict:
                                    if instrfield_value in instrfield_values_dict:
                                        for (
                                            syntax_value,
                                            option_name,
                                        ) in instrfield_values_dict[instrfield_value]:
                                            if (
                                                operand_values_dict[instrfield_value]
                                                == syntax_value
                                            ):
                                                # Registers
                                                result = mask & (
                                                    int(option_name) << range[1]
                                                )
                                    else:
                                        # Immediates
                                        ...
                                        if i == 0:
                                            mask = (
                                                (default_mask << int(shift))
                                                ^ (
                                                    default_mask
                                                    << (
                                                        int(shift) + int(range_diff) + 1
                                                    )
                                                )
                                            ) & 0xFFFFFFFF
                                            result = (
                ...
                with open(references_asm_file_path, "w") as references_asm_file:
                    ...
                    now = datetime.now()
                    references_asm_file.write(
                        f"# Copyright (c) {now.strftime('%Y')} NXP\n"
                    )
                    references_asm_file.write(
                        "# SPDX-License-Identifier: BSD-2-Clause\n\n"
                    )
