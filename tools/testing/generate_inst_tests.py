...
def write_header():
    ...
    try:
        ...
        print("Please run files.py first to generate the info module.")
        return
    ...
    (
        instruction_attribute_dict,
        new_instruction_attribute_dict,
    ) = parse.instruction_attribute(adl_file_path)
    ...
    for i, (instruction, operands) in enumerate(info.instructions.items()):
        ...
        if any(
            element in cmd_ext for element in instruction_attribute_dict[instruction]
        ):
            ...
            with open(file_name, "w") as f:
                ...
                f.write(
                    "# @brief          Encode %s %s" % (instruction, ",".join(operands))
                    + "\n"
                )
                f.write(
                    "# @details        Tests if each bit is encoded correctly for %s instruction"
                    % instruction
                    + "\n"
                )
                ...
                f.write("\n")
                f.write(
                    f'// RUN: %asm -arch={architecture} -mattr="{mattrib}" %s -o %s.o -filetype=obj\n'
                )
                f.write(f"// RUN: %readelf -x 2 %s.o | %filecheck reference.txt\n\n")
                f.write("\t.text\n")
                f.write("\t.attribute	4, 16\n")
                f.write(
                    f'\t.attribute	5, "{baseArchitecture}i{extension_versions_dict["i"]}'
                )
...
def generate_instructions():
    try:
        ...
        print("Please run generate_info.py first to generate the info module.")
        return None
    ...
    (
        instruction_attribute_dict,
        new_instruction_attribute_dict,
    ) = parse.instruction_attribute(adl_file)
    ...
    for file_path, (instruction, operands) in zip(
        sorted(file_paths), info.instructions.items()
    ):
        # generate tests only for specific extensions
        if any(
            element in cmd_ext for element in instruction_attribute_dict[instruction]
            ...
            # case if instruction has only register operands
                ...
                with open(file_path, "a") as f:
                    ...
                    for i, operand in enumerate(operands):
                        ...
                        # check if instruction is hint
                        if "_hint" in instruction:
                            other_operand_values = [
                                info.operands[op][0] for op in other_operands
                            ]
                        else:
                            other_operand_values = [
                                info.operands[op][-1] for op in other_operands
                            ]
                        if operand in widths_dict.keys():
                            ...
                            if found_constant:
                                f.write(
                                    f"\n#Testing operand {operand} encoded on {widths_dict[operand]} bits with {len(info.operands[operand][possible_value_1:possible_value_2])} values: {info.operands[operand][possible_value_1:possible_value_2]}\n"
                                )
                            else:
                                f.write(
                                    f"\n#Testing operand {operand} encoded on {widths_dict[operand]} bits with {len(info.operands[operand])} values: {info.operands[operand]}\n"
                                )
                        for j, value in enumerate(info.operands[operand]):
                            # check if any register operand is actually a constant value
                            found_constant = False
                            for tuple in instr_field_value_dict[instruction]:
                                if tuple[0] == operand and tuple[1] != None:
                                    value = info.operands[operand][2 * int(tuple[1])]
                                    found_constant = True
                                    break
                            ...
                            if "_hint" in instruction:
                                f.write(
                                    f"\t{instr_name_syntaxName_dict[instruction]} {','.join(instr_line)}\n"
                                )
                            else:
            ...
            # case if instruction has immediates and immediates with offsets
                ...
                with open(file_path, "a") as f:
                    ...
                    for key in operands_dict.keys():
                        operands_dict[key] = [
                            element.strip("()") for element in operands_dict[key]
                        ]
                    # check operand type
                    for op in operands_extended:
                        # if operand has offset and it's NOT a value from dict put its name between ()
                        if op in offsets and op not in operands_dict:
                            op_values = [op]
                        ...
                        elif op in offsets and op in operands_dict:
                            op_values = operands_dict[op]
                            for i in range(len(op_values)):
                                op_values[i] = "(" + str(op_values[i]) + ")"
                        # if operand is a register take the value from the dict
                        elif op in operands_dict:
                            # check if any register operand is actually a constant value
                            found_constant = False
                            for tuple in instr_field_value_dict[instruction]:
                        ...
                        # if operand is an immediate generate values based on its width
                            ...
                            if op in op_signExt_dict:
                                ...
                                if imm_signed_dict[op] == "true":
                                    op_values = [
                                        hex(2 ** (i + imm_shift_dict[op]))
                                        for i in range(
                                            0, width - imm_shift_dict[op] - 1
                                        )
                                    ] + [
                                        hex(
                                            2 ** (width - 1) - 2 ** (imm_shift_dict[op])
                                        )
                                    ]
                                    op_values += [
                                        hex(
                                            2 ** int(op_signExt_dict[op])
                                            - 2 ** (width - 1)
                                        ),
                                        hex(
                                            2 ** int(op_signExt_dict[op])
                                    ...
                                    op_values = [
                                        hex(2 ** (i + imm_shift_dict[op]))
                                        for i in range(
                                            0, width - imm_shift_dict[op] - 2
                                        )
                                    ] + [
                                        hex(
                                            2 ** (width - 2) - 2 ** (imm_shift_dict[op])
                                        )
                                    ]
                                    op_values += [
                                        hex(
                        ...
                        fixed_operands = [
                            f"({operands_dict[x][0]})"
                            if x in offsets
                            and x in operands_dict
                            and "_hint" in instruction
                            else f"({operands_dict[x][-1]})"
                            if x in offsets and x in operands_dict
                            else f"({x})"
                            if x in offsets
                            else operands_dict[x][0]
                            if x in operands_dict and "_hint" in instruction
                            else operands_dict[x][-1]
                            if x in operands_dict
                            else hex(
                                2 ** (imm_width_dict[x] - 1) - 2 ** (imm_shift_dict[x])
                            )
                            if x in imm_width_dict
                            and x in op_signExt_dict
                            and imm_signed_dict[x] == "true"
                            else hex(
                                2 ** (imm_width_dict[x] - 2) - 2 ** (imm_shift_dict[x])
                            )
                            if x in imm_width_dict and x in op_signExt_dict
                            else hex(
                                int(instrfield_value_double_dict[instruction][x], 16)
                            )
                            if x in imm_width_dict
                            and instruction in instrfield_value_double_dict
                            and x in instrfield_value_double_dict[instruction]
                            and instrfield_value_double_dict[instruction][x] is not None
                            else hex(
                                2 ** (imm_width_dict[x] - 1) - 2 ** (imm_shift_dict[x])
                            )
                            if x in imm_width_dict
                            else x
                            for x in operands_extended
                            if x != op
                        ]
                        # information displayed about each type of operand
                        if op in offsets:
                            if op in operands_dict:
                                f.write(
                                    f"\n#Testing offset {op} encoded on {widths_dict[op]} bits with {len(info.operands[op])} values: {info.operands[op]}\n"
                                )
                            else:
                            ...
                            if found_constant:
                                f.write(
                                    f"\n#Testing operand {op} encoded on {widths_dict[op]} bits with {len(info.operands[op][possible_value_1:possible_value_2])} values: {info.operands[op][possible_value_1:possible_value_2]}\n"
                                )
                            else:
                                f.write(
                                    f"\n#Testing operand {op} encoded on {widths_dict[op]} bits with {len(info.operands[op])} values: {info.operands[op]}\n"
                                )
                        else:
