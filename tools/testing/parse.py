...
def instructions_operands(adl_file):
    ...
    selected_keys = {
        key
        for key in llvm_config_dict.keys()
    ...
    for cores in root.iter("cores"):
        ...
        for instruction in cores.iter("instruction"):
            if (instruction.find("pseudo")) or (instruction.find("alias_action")):
                ...
                for attribute in instruction.iter("attribute"):
                    if any(
                        attribute.get("name") == str.lower(llvm_config_dict[key])
    ...
    filtered_instr_op_dict = {
        key: instr_op_dict[key] for key in only_take_attributes if key in instr_op_dict
    }
...
def operands_values(adl_file):
        ...
        for instrfield in cores.iter("instrfield"):
            ...
            for enumerated in instrfield.iter("enumerated"):
                for option in enumerated.iter("option"):
                    if (
                        option.find("str").text is not None
                        and option.find("str").text != "reserved"
                    ):
                        option_name_list.append(option.get("name"))
                        reg_value_list.append(option.find("str").text)
            instrfield_optionName_dict[instrfield_name] = [
                (op_name, reg_val)
                for (op_name, reg_val) in zip(option_name_list, reg_value_list)
            ]
    ...
    for operand, tuples in instrfield_optionName_dict.items():
        ...
        if width == 0:
            ...
            filtered_symbols = [
                symbol
                for option_name, symbol in tuples
                if offset <= int(option_name) <= (offset + 2**width - 1)
            ]
    ...
    # extract the registers between () and separate elements by whitespaces
    extracted_inputs_dict = {
        key: [
            re.search(r"\((.*?)\)", value).group(1)
            for value in values
            if re.search(r"\((.*?)\)", value)
        ]
        for key, values in instruction_inputs_dict.items()
    }
    extracted_inputs_dict = {
        key: [elem for value in value_list for elem in value.split()]
        for key, value_list in extracted_inputs_dict.items()
    }
    extracted_outputs_dict = {
        key: [
            re.search(r"\((.*?)\)", value).group(1)
            for value in values
            if re.search(r"\((.*?)\)", value)
        ]
        for key, values in instruction_outputs_dict.items()
    }
    extracted_outputs_dict = {
        key: [elem for value in value_list for elem in value.split()]
        for key, value_list in extracted_outputs_dict.items()
    }

    # keep only the instructions with register pairs
    filtered_inputs_dict = {
        key: values
        for key, values in extracted_inputs_dict.items()
        if any(values.count(item) > 1 for item in final_op_val_dict.keys())
    }
    filtered_outputs_dict = {
        key: values
        for key, values in extracted_outputs_dict.items()
        if any(values.count(item) > 1 for item in final_op_val_dict.keys())
    }

    instruction_registerPair_dict = {**filtered_inputs_dict, **filtered_outputs_dict}
...
def instruction_attribute(adl_file):
    ...
    new_instruction_attribute_dict = {
        instruction: [
            attr[len(base_architecture) :]
            if attr.startswith(base_architecture)
            else attr
            for attr in attributes
        ]
        for instruction, attributes in instruction_attribute_dict.items()
    }
