...
def write_header():
    ...
    for i, (instruction, operands) in enumerate(instr_op_dict.items()):
        ...
        if cmd_extensions is None or all(extension in instruction_attributes_dict[instruction] for extension in cmd_extensions):
            ...
            with open(file_name, "w") as f:
                ...
                for extension in norv32_instruction_attributes_dict[instruction]:
...
def generate_instructions():
    ...
    for file_path in file_paths:
        ...
        for instruction, operands in instr_op_dict.items():
            # Generate tests only for specific extensions
            if instruction in path_components and (cmd_extensions is None or all(extension in instruction_attributes_dict[instruction] for extension in cmd_extensions)):
