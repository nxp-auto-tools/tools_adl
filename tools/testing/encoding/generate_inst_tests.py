                ...
                with open(file_path, 'a') as f:
                    ...
                    for i, operand in enumerate(operands):
                        ...
                        if '_hint' in instruction:
                            other_operand_values = [info.operands[op][0] for op in other_operands]
                        else:
                            other_operand_values = [info.operands[op][-1] for op in other_operands]
            ...
            # case if instruction has immediates and immediates with offsets
                ...
                with open(file_path, 'a') as f:
                    ...
                    for key in operands_dict.keys():
                        operands_dict[key] = [element.strip(
                            '()') for element in operands_dict[key]]                
                    # check operand type
                    for op in operands_extended:
                        ...
                        # if operand has offset and it's a value from dict put its value between ()
                            ...
                            if instruction in instruction_register_pair_dict and op in instruction_register_pair_dict[instruction]:
                                ...
                                op_values = [value for value in pair_value_list if value in operands_dict[op]]
                            else:
                                op_values = operands_dict[op]
                            for i in range(len(op_values)):
                                op_values[i] = '(' + str(op_values[i]) + ')'
                        # if operand is a register and has paired values
                        elif op in operands_dict and instruction in instruction_register_pair_dict and op in instruction_register_pair_dict[instruction]:
                            pair_value_list = []
                            for val_optName_tuple in instrfield_optionName_dict[op]:
                                # take only even values
                                if int(val_optName_tuple[0]) % 2 == 0:
                        ...
                        # if operand is just a simple register take the value from the dict
                        elif op in operands_dict and instruction not in instruction_register_pair_dict:
                            # check if any register operand is actually a constant value
                            found_constant = False
                            for tuple in instr_field_value_dict[instruction]:
                            ...
                            # if there is a register with constant value, take only the possible values   
                            if found_constant:
                                op_values = operands_dict[op][possible_value_1:possible_value_2]
                            else:
                        ...
                        fixed_operands = [f"({operands_dict[x][0]})" if x in offsets and x in operands_dict and '_hint' in instruction
                                        else f"({operands_dict[x][-1]})" if x in offsets and x in operands_dict and (int(instrfield_optionName_dict[x][-1][0]) % 2 == 0) and instruction in instruction_register_pair_dict
                                        else f"({operands_dict[x][-3]})" if x in offsets and x in operands_dict and (int(instrfield_optionName_dict[x][-1][0]) % 2 != 0) and instruction in instruction_register_pair_dict
                                        else f"({operands_dict[x][-1]})" if x in offsets and x in operands_dict
                                        else f"({x})" if x in offsets
                                        else operands_dict[x][0] if x in operands_dict and '_hint' in instruction
                                        else operands_dict[x][-1] if x in operands_dict and (int(instrfield_optionName_dict[x][-1][0]) % 2 == 0) and instruction in instruction_register_pair_dict
                                        else operands_dict[x][-3] if x in operands_dict and (int(instrfield_optionName_dict[x][-1][0]) % 2 != 0) and instruction in instruction_register_pair_dict
                                        else operands_dict[x][-1] if x in operands_dict
                        ...
                        if op in offsets:
                            ...
                            if op in operands_dict and instruction in instruction_register_pair_dict and op in instruction_register_pair_dict[instruction]:
                                f.write(
