...
def instructions_fields(adl_file):
    ...
    for cores in root.iter("cores"):
        for instruction in cores.iter("instruction"):
            if instruction.get("name") in instr_op_dict and instruction.find("aliases") is None:
                ...
                for field in instruction.iter("field"):
                    if field.find("str") is not None:
                        if field.find("str").text is not None:
                            instructions_field_value_tuple_list.append((field.get("name"), field.find("str").text))
                        else:
                            instructions_field_value_tuple_list.append((field.get("name"), field.get("name")))
                    else:
                        instructions_field_value_tuple_list.append((field.get("name"), field.find("int").text))
                instruction_fields_dict[instruction.get("name")] = instructions_field_value_tuple_list
    ...
    for cores in root.iter("cores"):
        for instruction in cores.iter("instruction"):
            if instruction.get("name") in instr_op_dict and instruction.find("aliases"):
                ...
                for field, value in zip(instruction.iter("field"), instruction.iter("value")):
                        if value.find("str") is not None:
                            aliases_field_value_tuple_list.append((field.find("str").text, value.find("str").text))
                        else:
                            aliases_field_value_tuple_list.append((field.find("str").text, value.find("int").text))
                alias_fields_dict[instruction.get("name")] = aliases_field_value_tuple_list

    # Create a dict with only field names in order to delete duplicate field names
    alias_field_names_dict = {instruction: [t[0] for t in operand_value_tuples] for instruction, operand_value_tuples in alias_fields_dict.items()}
    ...
    for alias, instruction in alias_instruction_dict.items():
        if instruction in instruction_fields_dict and alias in alias_fields_dict:
            ...
            for instruction_tuple in instruction_tuples:
                matching_alias_tuple = next((alias_tuple for alias_tuple in alias_tuples if alias_tuple[0] == instruction_tuple[0]), None)
                if matching_alias_tuple:
                    swapped_alias_values.append((instruction_tuple[0], matching_alias_tuple[1]))
                else:
                    swapped_alias_values.append((instruction_tuple[0], instruction_tuple[1]))

            # Check if there are unmatched alias tuples
            unmatched_alias_tuples = [alias_tuple for alias_tuple in alias_tuples if all(alias_tuple[0] != instr_tuple[0] for instr_tuple in instruction_tuples)]
    ...
    final_instruction_fields_dict = {**instruction_fields_dict, **aliases_with_instruction_values_dict}
    final_instruction_fields_dict = {key: final_instruction_fields_dict[key] for key in sorted(final_instruction_fields_dict)}
                ...
                if instrfield.find("enumerated"):
                    for enumerated in instrfield.iter("enumerated"):
                        ...
                        for option in enumerated.iter("option"):
                            option_name_value_list.append((option.find("str").text, option.get("name")))
                        instrfield_values_dict[instrfield.get("name")] = option_name_value_list
