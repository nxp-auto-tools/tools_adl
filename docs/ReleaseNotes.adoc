...
:le: &#8804;
:rarr: &#8658;
:title-logo-image: image:tristan_rigoletto_logo.png[]

== Intro

This project is part of European projects Tristan and Rigoletto.
...
| 0.4     | 16-02-2026 | Updated Release Notes for WP4_WP6_M39 release     
|===


== Overview

This project comprises three components aimed at facilitating the automatic generation and testing of LLVM target description files based on the information parsed from an architectural description of a RISC-V core.
...
* The third component consists in a Sail to ADL converter, which allows to convert Sail specifications into ADL format for further processing and integration.

More information on ADL technology can be found at https://github.com/nxp-archive/adl-tools_adl/


== What Is Tools-ADL Project?

Tools-ADL is a tool suite designed for automatically generating target description files and encoding tests using an XML file. Practically, Tools-ADL parses an XML file, which contains an ADL model for one or more RISC-V extensions, and generates all the necessary .td files required by a compiler to integrate these new extensions. In simpler terms, the ADL represented as an XML file is crucial in this process, alongside the actual tools used.

== Requirements and Installation

* *python 3.10* or higher
* *llvm-project* - integration and running test verification with llvm-lit (https://github.com/llvm/llvm-project - release/19.x) 
...
For developers (including development dependencies):
...
NOTE: All required dependencies (numpy, num2words, word2number) will be installed automatically.

NOTE: The generated files have been included in the *llvm-project (release 19.x)* and were tested and validated against the same LLVM version.
...
To run the application, the user has to use *make_td.py* module
which is the main module that triggers the other modules. Besides
calling this module, the user also must provide a proper XML file
...
There are no constraints regarding the location for running make_td.py, as a 
result the user should be able to run it from any location, providing that the path to the XML file is correct. 
...
which extensions should be generated. Besides specifying *the extensions enabled*,
...
NOTE: Sail version used is 0.10. For more information on Sail version, please refer this https://github.com/riscv/sail-riscv/releases/tag/0.10[link].
NOTE: If *no extension is specified*, then the tool generates content
...
the rv32i XML model is ignored and nothing related to it is generated.
The flag to enable extension generation is *--extension=*, while for specifying the output directory is
*--output=*.
...
Furthermore, for enabling 64 bit support, there is an option in the command line for enabling 64 bit support generation.

	python ./tools/make_td.py rv32i.adl.xml --target=riscv64 --extension=zclsd,zilsd,rv32i --output=./examples/TD/test_zilsd

By enabling riscv64 option, tools_adl will generate the support for register description and instruction description .td files.  

=== Testing module

NOTE: Running the script directly as in the example below requires an active Python virtual environment (venv).
This is because the script relies on the project’s package structure and editable installation (pip install -e .), which are correctly set up only when the venv is activated.
Without an active venv, Python may not resolve project modules properly, and imports can fail.
...
- For *fixup tests* the user must firstly set up the environment by running the following command:
...
To understand the content and structure of llvm-config.txt, we
...
the path or the folder (depending on the case) where the content is generated. The name given to a certain td file should preserve LLVM format name, similar to those listed below.
	...
	ScheduleFile = RISCVSchedule_gen.td
	
	ScheduleFileTable = RISCVScheduleTable_gen.td
...
** This section includes information about environmental variables or
...
** The user can define a certain register class.
	...
	RegisterClass = {GPR=RISCVReg, VR=RISCVReg}
...
** This tag specifies the description for the vector register class

	VectorRegisterClass = {ClassName=VReg, ParentClass=RISCVRegisterClass, Width=64, VLMul=Vlmul, VLMulValue=1, IsVRegClass=1, Parameters=[VM1VTs,VMaskVTs]}
...
instructions, there are a few instruction types defined. The left value is
...
* *Calling Convention*
** This section contains calling convention information. It specifies
...
entries from this section specify additional information.

	RegisterAllocationOrder = {GPR: [(Function_arguments, [0,1]), (Temporary, [0,1]), (Saved_register, [0,1]), Hard_wired_zero, (Return_address, [0,1]), (Stack_pointer, [0,1]), (Global_pointer, [0,1]), (Thread_pointer, [0,1])], VR: [Higher, Lower_7, Lower_6, Lower_5, Lower_4, Lower_3, Lower_2, Lower_1, Lower_0]}
...
** Also, more information for register pairs is now required

	RegInfosPair = RegInfoByHwMode<[RV32, RV64], [RegInfo<64, 64, 32>, RegInfo<128, 128, 64>]>
...
it should be generated, then it must be defined in this section,
...
declarations which cannot be automatically generated with the
...
As an example, for Zilsd instructions the DecoderNamespace has to be Zilsd

	
	DecoderNamespace = {Zilsd=Zilsd}

Moreover, some instructions need a specific prefix to be correctly decoded and generated. The prefix information is stored in the ExtensionPrefix 
dictionary where the keys represent the extension names and the values are the prefix strings to be used. This ensures that similar instruction from other
extensions will be correctly encoded and decoded without conflicts.

	ExtensionPrefixed = {Zclsd, Zilsd}

Besides the configuration file, the user should add some important information in the XML model.

* *Instruction field definition* 

Firstly, the user should provide new instruction fields for special register subclasses.
...
The instruction field should be like other instruction filed already defined.
...
or not for proper usage of the tools. The file contains all the tags
...
*** *<pcrel>* (str) – Optional, whether this is a pc-relative
relocation. 
...
arguments
*** *<intrinsic_type>* – List used for defining argument types for
intrinsic
...
find specific information in the XML file parsed. To activate this feature
the user must make sure that the XML file specifies in *<inputs>* and/or *<outputs>* tags
...
as pairs. Firstly, the tools generate consecutive pairs of registers, indicating information about
...
and based on the register class *width* and *shift* values as well.
...
any ADL model given as input argument. To activate this
feature, the user should be aware of the information required for proper
generation. The tools can generate instructions patterns,
...
To use all these features, the user has to provide several
information in the ADL xml model as follows:
...
with instructions for which the <intrinsic> tag is defined.
...
This tag takes each argument previously defined and specifies data
...
This identifier is then used in several generated files as follows:
...
The files generated contain all the details about intrinsic and
...
*Naming convention* is also handled by these tools. To ease the
...
definition to pass the validation. The tests are automatically
generated in a customized folder which is generally included in
...
Sail description generation is a feature supported in ADL tools for having, beside TD files generation, the **RISC-V** extension formal specification written in Sail for instruction enconding, semantic and parsing/decoding information. 
	...
	* *How it works?*
		** To enable Sail description generation, the user should run the basic **make-td** command line. When this command is run, a **.sail** file is generated for each RISC-V extension which is specified. 
...
NOTE: For SAIL the user must set the extensions using the --extension flag. The order of the attributes from the command line matters. 
If an instruction has more than one attribute it will only be generated once for the first attribute provided at --extension flag (this may result in empty files being generated). 
	...
	* *How a Sail description looks?*
		...
		** Each instruction supported contains 4 parts defined: union clause instruction, mapping clause encdec, function clause execute, mapping clause assembly.
		** For defining an extension, there is *function clause extension definition* where the extension enabled is defined.
				
NOTE: Function clause definition is not defined for I extension, only for other extensions such as C Extension, Zilsd Extension etc 

				function clause currentlyEnabled(Ext_Zilsd) = hartSupports(Ext_Zilsd)

		** Beside extension definition, other dependencies are defined (e.g. other extensions which are required). Any extension which represents a dependecy is then generated with *currentlyEnabled()*, except for *rv32i* extension definition.
		** *hartSupports()* definition is also required for the extension defined, except for *rv32i* extension.

			function clause currentlyEnabled(Ext_C) = hartSupports(Ext_C) & function clause currentlyEnabled(Ext_Zilsd) = hartSupports(Ext_Zilsd)

		** Union clause instruction is a formal description for the instruction mnemonic in which it also specifies which type of instruction fields is used (registers or immediates).

				union clause instruction = ADD : (regidx, regidx, regidx)
...
NOTE: For other extension than rv32i, currentlyEnabled() definitions will be generated for activating the correct extension:
...
ADL tools project can generate scheduling description for several extensions. This tool generates a scheduling table description, which contains more information about the core, but also provides scheduling information for each instruction from the extension supported. 
	...
	* *How it works?*
		...
		** The XML contains <sched-table> tag which provides a high-level description for a scheduling model. It specifies the model type, functional units, instruction types, instrution latency and throughput and other scheduling options.

NOTE: All the fields listed below map on *LLVM MCSchedModel* structure from *MCSchedule.h* file. Generally, *each field* from this *scheduling model* suggests which field from *MCSchedModel* structure is set. 
...
NOTE: *CompleteModel* *(This field maps llvm::MCSchedModel::LoadLatency)* and *UnsupportedFeatures*(defined in *SchedMachineModel* class from *llvm/Target/TargetSchedule.td*) have to be set manually by the user. 
		...
		** *_instruction-sched_ contains instruction types, latency, throughput and functional units used*
...
NOTE: More information about this tag can be found in *llvm/include/llvm/Target/TargetSchedule.td* file.
...
NOTE: More information about this tag can be found in *llvm/include/llvm/Target/TargetSchedule.td* file.
			...
			*** *These tags below provide informations about forwarding such as the resources for which it applies, and the value used for ReadAdvance*
				...
				**** *In case of a resource group, this tag allows the user to specify the resources that must be grouped*
...
NOTE: Scheduling description is entirely based on the information parsed from *<sched-table>* tag and it should be correctly and completely defined for a proper generation. 
	...
	*	*How scheduling description looks?*
		...
		** Naming convention for these scheduling files could be set in *llvm-config* by editing *ScheduleFile*, *SchedulePath* and *ScheduleFileTable* variables, providing information about the path and the file's name. 
	...
	* *Scheduling tests integrating llvm-mca and llvm-lit*
		** Once the scheduling description model is done, it should be tested and validated to avoid possible issues. 
		...
		** That is the reason behind generating scheduling tests. ADL tools project provides scheduling tests for each instruction parsed from the XML file. 
...
NOTE: *CHECK* and *CHECK-NEXT* are commands for llvm-lit which specify the execution order
		...
		** Basically, *llvm-mca* runs the instruction and generates an execution timeline which is compared to the reference using *llvm-lit*.
...
NOTE: This command line shows the result from every single test from the test suite run and provides a *.txt* file which serves as log, for more information about certain tests. 


== Sail Converter

Sail conveter is a tool dedicated for translatig Sail description into ADL XML format, used as input for the ADL tools project. 
Basically, the translator parses the Sail specification and extracts the relevant information to generate an ADL-compatible XML file.
The information parsed in converted into structured XML elements that represent instruction definition, register definition, instruction fields,
instruction encoding, sign extension information, asm_config parameters and calling convention information. 
The Sail specification is generated as JSON file once SAIL-RISCV project is built.

=== Command line

	python .\sail2adl.py <sail_model.json> --extensions Extension1 Extension2 --output <path for the output folder>

The first paramereter represents the Sail model, as it is generated in SAIL-RISCV project. 
The second parameter specifies the extensions to be included in the conversion.
The third parameter represents the output folder where the XML file resulted will be generated.

The XML file generated has the same syntax and structure as the one used by ADL tools project, allowing seamless integration with the existing toolchain. 
Beside the XML generated, the tool produces also a log file with conversion details and warnings for any issue encountered during the translation process, but 
also contains information about the translation process. The log file is generated in debug/ folder. 

As proof of concept, as part of the collaboration with IMT, Sail converter was used for generating the ADL model in XML format based on the Sail description.
Sail description was also generated from an initial manually written ADL which served as a basis. So, the flow is ADL -> SAIL -> ADL, both ADL models being identical. 
All the support and implementation for IMT extension is uploaded on https://github.com/alex2kameboss/MA-sail-riscv/tree/master/model/extensions/Zimt[GitHub].


NOTE: Sail converter is compliant with version 0.10. For more information about Sail version, please refer https://github.com/riscv/sail-riscv/releases/tag/0.10. 

== Known Limitations
	* *Fix-up relocations not supported*
	* *Scheduling information is for demo purposes*
	* *Sail converter is for demo purposes*
